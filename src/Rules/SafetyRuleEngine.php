...
namespace MigrationSafe\Laravel\Rules;

use MigrationSafe\Laravel\Analysis\AnalysisContext;
use MigrationSafe\Laravel\Database\DatabaseInspector;
...
final class SafetyRuleEngine
{
    ...
    public function analyze(MigrationOperation $operation, AnalysisContext $context, string $migration): array
    {
        ...
        $enabled = config('migration-safety.rules');
        ...
        return match ($operation->type) {
            ...
            'add_column' => !$operation->nullable && ($tableRows ?? 0) > 0 && $enabled['add_not_null_column'] ? [$risk(RiskLevel::High, 'add_not_null_column', "Adds a non-nullable column to a populated table ({$tableRows} estimated rows).", 'Add it as nullable or with a safe default, backfill, then enforce NOT NULL.')] : [$risk(RiskLevel::Low, 'add_column', 'Adds a column without a detected populated-table constraint.', 'Verify application compatibility before deployment.')],
            'add_index' => $this->indexRisk($operation, $tableRows, $risk, 'large_table_index', 'Adds an index to a large table.'),
            ...
            'rename_column' => [$risk(RiskLevel::Medium, 'rename_column', 'Renames an existing column.', 'Deploy compatible application code before removing references to the old name.')],
    ...
    private function indexRisk(MigrationOperation $operation, ?int $rows, \Closure $risk, string $rule, string $reason): array
    {
        if (!config('migration-safety.rules.large_table_index') || ($rows ?? 0) < config('migration-safety.thresholds.large_table_rows')) {
            return [$risk(RiskLevel::Low, 'add_index', 'Adds an index.', 'Verify the operation against expected table size.')];
        }

        return [$risk(RiskLevel::High, $rule, "{$reason} {$rows} estimated rows.", 'Use an online index strategy and test its locking behavior.')];
    }
    ...
    private function uniqueRisk(MigrationOperation $operation, AnalysisContext $context, string $migration, \Closure $risk): array
    {
        ...
        return $this->indexRisk($operation, $this->inspector->estimatedRows($operation->table, $context->connection), $risk, 'large_table_unique_index', 'Adds a unique index to a large table.');
    }
